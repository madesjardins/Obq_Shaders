...
def Obq_MeasuredMaterialHelpURL():
	...
	ObqNodeHelpURL = 'http://s3aws.obliquefx.com/public/shaders/help_files/Obq_MeasuredMaterial.html'
...
class AEObq_MeasuredMaterialTemplate(ShaderAETemplate):
	...
	def setup(self):
		...
		self.addControl("direct_color", label="Color")
		self.addControl("direct_scale", label="Scale")
		self.addControl("useMIS", label="Use MIS (test)")
		self.endLayout() # End Direct
		self.beginLayout("Indirect", collapse=False )
		self.addControl("indirect_color", label="Color")
		self.addControl("indirect_scale", label="Scale")
		self.addControl("useImportanceSampling", label="Use importance sampling (test)")
		self.addControl("samples", label="Resolution")
		...
		self.addControl("direct_AOV", label="Direct")
		self.addControl("indirect_AOV", label="Indirect")
		self.endLayout()
