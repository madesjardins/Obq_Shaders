...
def createLinkNewTab(address, message):
    return '<a href="' + address + '" target="_blank">' + message + '</a>'
...
def writeContentSidebar(file, proj=0, cat=0):
    ...
    for helpfile in aHelpFiles:
        ...
        file.write(
            '<tr><td width="2"></td><td>' + createLink(prefix + shaderName + ".html", shaderName) + '</td></tr>\n')
...
def writeContentHelp(file, proj=0, cat=0, helpfile=""):
    ...
    if cat == 0:
        ...
        file.write('<p>A collection of Arnold shaders created by Marc-Antoine Desjardins at ' + createLink(
            "http://www.obliquefx.com", "Oblique FX") + '.</p>\n')
        ...
        with open("src/info_builds.txt", 'r') as f:
            lines = f.readlines()
            file.write('<p>' + createLink(PATH_WEBSITE + PATH_BUILDS + cleanStr(lines[0]),
                                          cleanStr(lines[1]).replace(" ", "&nbsp;")) + " [" + cleanStr(
                lines[2]) + "] " + createLink(PATH_WEBSITE + PATH_RN + cleanStr(lines[3]), "Release Notes") + '</p>\n')
            f.close()
        ...
        # br(file)
        file.write('<h3>Source Code</h5>\n')
        file.write('<p>' + createLinkNewTab("https://github.com/obliquefx/Obq_Shaders", "Obq_Shaders Github repository") + '.</p>\n')
        ...
        with open(helpfile, 'r') as f:
            ...
            for line in lines:
                ...
                if sublines[0].find("%shadername") != -1:
                    ...
                    # Not Anymore # first we need to copy
                    br(file)
                ...
                # br(file)
                    ...
                    for i in range(2, len(sublines)):
                        if i != 2:
                            ...
                            file.write('<td valign="top" align="left" width="' + cleanStr(
                                sublines[1]) + '"><span class="enumElem">' + cleanStr(sublines[i]) + '</span></td>')
                    file.write('</tr>\n')
    ...
    # DOCUMENTATION
        ...
        with open("src/INSTALL.html", 'r') as f:
            lines = f.readlines()
            for line in lines:
                if line.find("DOCTYPE") == -1 and line.find("title>") == -1 and line.find("head>") == -1 and line.find(
                        "body>") == -1:
                    file.write(line)
            f.close()
        br(file)
    ...
    # Downloads
        ...
        with open("src/info_builds.txt", 'r') as f:
            ...
            for listID in range(1, (len(lines) + 1) / 5):
                file.write('<p>' + createLink(PATH_WEBSITE + PATH_BUILDS + cleanStr(lines[5 * listID]),
                                              cleanStr(lines[5 * listID + 1]).replace(" ", "&nbsp;")) + " [" + cleanStr(
                    lines[5 * listID + 2]) + "] " + createLink(PATH_WEBSITE + PATH_RN + cleanStr(lines[5 * listID + 3]),
                                                               "Release Notes") + '</p>\n')
                br(file)
            br(file)
            br(file)
        ...
        file.write('<p>The Obq_Shaders collection will only grow stronger with the help of its users. Please address your questions, feature requests and bug reports to <span style="color:darkblue;">marcantoinedesjardins(AT)gmail.com</span>.</p>')
